...
# --- Payment-at-Registration Schemas (Owner Onboarding Billing) ---

class PlanSelectionRequest(BaseModel):
    plan: str = "monthly"  # monthly / quarterly / yearly

class PlanPrices(BaseModel):
    monthly: Decimal = Decimal("1500.00")
    quarterly: Decimal = Decimal("4000.00")
    yearly: Decimal = Decimal("14400.00")

class OwnerPaymentInitRequest(BaseModel):
    gym_id: int
    plan: str = "monthly"
    amount: Decimal = Decimal("1500.00")

class OwnerPaymentVerifyRequest(BaseModel):
    gym_id: int
    utr_number: str
...
class OwnerPaymentStatusResponse(BaseModel):
    gym_id: int
    business_name: str
...
# --- Doctor / Clinic Availability Schemas ---

class DoctorAvailabilityUpdate(BaseModel):
    working_days: Optional[str] = None       # e.g. "Mon-Sat"
    start_time: Optional[str] = None         # e.g. "10:00 AM"
    end_time: Optional[str] = None           # e.g. "02:00 PM"
    second_start_time: Optional[str] = None  # optional 2nd session
